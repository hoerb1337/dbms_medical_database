...
and shared_meds.nr_shared_meds > 1)full_table
        ...
        if st.button(label="Execute Analysis"):
            ...
            if result_analysis == "False":
                result_display = "<div class='result_box_positive'><h4>Result of Analysis: No</h4><p>No, drugs with shared proteins do not - on average - have common side effects. On average one side effect of drugs with a shared protein is common only in <b>" + avg_ratio_se_meds + "</b> of all drugs with this shared protein.</p></div>"   
